...
const targets = computed(() => {
  ...
  const out: { label: string, value: string, icon: string }[] = [{ label: 'Toàn máy (mọi project)', value: '__user', icon: 'i-lucide-monitor' }]
  for (const p of projData.value?.projects ?? []) {
    if (!p.path || seen.has(p.path)) continue
    seen.add(p.path)
    out.push({ label: `${p.name} · project office`, value: p.path, icon: 'i-lucide-folder-git-2' })
  }
...
const where = computed(() => {
  if (target.value === '__user') {
    return props.source?.kind === 'mcp' ? 'Thêm vào cấu hình Claude Code của máy (~/.claude.json)' : `~/.claude/${props.source?.kind === 'skill' ? 'skills' : 'agents'}/`
  }
  if (props.source?.kind === 'mcp') {
    return mcpScope.value === 'project' ? `${target.value}/.mcp.json (commit vào git, cả team dùng)` : 'Chỉ máy này, cho riêng project này (không commit)'
  }
  return `${target.value}/.claude/${props.source?.kind === 'skill' ? 'skills' : 'agents'}/`
})
...
async function install() {
  ...
  try {
    ...
    toast.add({ title: `Đã cài ${name.value}`, description: res.path, color: 'success', icon: 'i-lucide-circle-check' })
    open.value = false
    emit('done')
  } catch (e) {
...
<template>
  <UModal v-model:open="open" :title="source ? `Cài ${kindMeta[source.kind].label}: ${source.name}` : ''" :ui="{ content: 'max-w-xl' }">
    <template #body>
      <form v-if="source" id="install-form" class="space-y-4" @submit.prevent="install">
        <UFormField v-if="!fixedTarget" label="Cài vào">
        ...
        <URadioGroup
          ...
          v-model="mcpScope"
          :items="[
            { label: 'Chia sẻ trong project', description: 'Ghi vào .mcp.json, commit để cả team dùng', value: 'project' },
            { label: 'Riêng máy này', description: 'Không commit, hợp với MCP có token cá nhân', value: 'local' }
          ]"
        />
        ...
        <template v-if="inputs.length">
          <USeparator label="Thông tin cần điền" />
          <UFormField
            v-for="i in inputs" :key="i.key"
            :label="i.label" :required="i.required" :help="i.hint"
            :description="i.kind === 'env' ? 'Biến môi trường' : i.kind === 'header' ? 'HTTP header' : 'Tham số'"
          >
          ...
          <p v-if="inputs.some(i => i.secret) && mcpScope === 'project' && target !== '__user'" class="text-xs text-(--ui-warning)">
            ...
            Token sẽ nằm trong .mcp.json và vào git. Nên chọn "Riêng máy này" hoặc dùng biến dạng <code>${TÊN_BIẾN}</code>.
          </p>
        </template>
        <UAlert v-if="template?.auth === 'oauth'" color="info" variant="subtle" icon="i-lucide-log-in" title="Đăng nhập khi dùng lần đầu" description="Server này dùng OAuth: Claude Code sẽ mở trình duyệt để đăng nhập (hoặc gõ /mcp trong Claude Code)." />

        <div v-if="findings.length" class="space-y-1 rounded-md border border-(--ui-border) p-3">
          <p class="text-sm font-medium">Kiểm tra an toàn</p>
        ...
        <UCheckbox v-if="needs === 'exists'" v-model="overwrite" label="Ghi đè (bản cũ được chuyển vào thùng rác)" />
        <UCheckbox v-if="needs === 'consent'" v-model="accept" label="Tôi đã xem các cảnh báo, vẫn cài" />
      </form>
    </template>
    <template #footer>
      <div class="flex w-full justify-end gap-2">
        <UButton color="neutral" variant="ghost" label="Hủy" @click="open = false" />
        <UButton type="submit" form="install-form" icon="i-lucide-download" label="Cài đặt" :loading="busy" :disabled="needs === 'unsafe'" />
      </div>
    </template>
  </UModal>
</template>
